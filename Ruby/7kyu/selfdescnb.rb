...
A number is self-descriptive when the n'th digit describes the amount n appears
in the number.
...
Numbers can be of any length up to 9 digits and are only full integers. For a
given number derive a function selfDescriptive(num) that returns; true if the
number is self-descriptive or false if the number is not.
