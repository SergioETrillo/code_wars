...
def mean(town, strng)
  # your code
  return 0 if town =="" or strng==""
  cities = []
  temp = []
  lines = strng.lines("\n")
  lines.each do |e|
      cities << e.scan(/\A[A-Z][A-Za-z]+/)
      temp << e.scan(/\d+.\d/)
  end
  return -1 unless cities.include?([town])
  index = cities.find_index([town])
  temp_nbs = temp[index].collect {|e| e.to_f }
  average = temp_nbs.reduce(:+)/temp[index].length
end
...
def variance(town, strng)
  ...
  temp_nbs = temp[index].collect {|e| e.to_f }
  average = temp_nbs.reduce(:+)/temp[index].length
  variance = temp_nbs.inject(0) {|res, n| res +(n-average)**2 }/temp_nbs.length
end
...
def extract_data(town, str)
  raw = str.lines.select { |line| line.start_with? "#{town}:" }
  raw[0].scan(/(?<=\:).+(?=\n)*/)[0].split(",").map { |i| [i.split[0], i.split[1].to_f] }.to_h rescue nil
end

def mean(town, str)
  data = extract_data(town,str)
  return -1 unless data
  data.values.reduce(:+).fdiv(data.size)
end

def variance(town, str)
  data = extract_data(town,str)
  return -1 unless data
  data.values.map { |i| (i-mean(town,str))**2 }.reduce(:+).fdiv(data.size)
end
